...
function poste_deux_upgrade($nom_meta_base_version, $version_cible) {
	...
	$maj['create'] = array(array('maj_tables', array('spip_facts', 'spip_contacts')));
...
function poste_deux_vider_tables($nom_meta_base_version) {
	...
	sql_delete("spip_versions",              sql_in("objet", array('fact', 'contact')));
	sql_delete("spip_versions_fragments",    sql_in("objet", array('fact', 'contact')));
	sql_delete("spip_forum",                 sql_in("objet", array('fact', 'contact')));
